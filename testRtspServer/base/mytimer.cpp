...
MyTimer::MyTimer(const std::string sTimerName):m_bExpired(true), m_bTryExpired(false), m_bLoop(false)
{
    m_sName = sTimerName;
}
...
bool MyTimer::Start(unsigned int msTime, std::function<void()> task, bool bLoop, bool async)
{
