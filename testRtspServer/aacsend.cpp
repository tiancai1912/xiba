...
int AACSend::bindSocketAddr(int sockfd, const char* ip, int port)
{
...
int AACSend::parseAdtsHeader(uint8_t* in, struct AdtsHeader* res)
{
...
int AACSend::rtpSendAACFrame(int socket, const char* ip, int16_t port,
...
int AACSend::acceptClient(int sockfd, char* ip, int* port)
{
    int clientfd;
    socklen_t len = 0;
    struct sockaddr_in addr;
...
char* AACSend::getLineFromBuf(char* buf, char* line)
{
    while(*buf != '\n')
    {
        *line = *buf;
...
int AACSend::handleCmd_OPTIONS(char* result, int cseq)
{
...
int AACSend::handleCmd_DESCRIBE(char* result, int cseq, char* url)
{
    char sdp[500];
    char localIp[100];
...
int AACSend::handleCmd_SETUP(char* result, int cseq, int clientRtpPort)
{
    sprintf(result, "RTSP/1.0 200 OK\r\n"
                    "CSeq: %d\r\n"
...
int AACSend::handleCmd_PLAY(char* result, int cseq)
{
    sprintf(result, "RTSP/1.0 200 OK\r\n"
                    "CSeq: %d\r\n"
                    "Range: npt=0.000-\r\n"
...
void AACSend::doClient(int clientSockfd, const char* clientIP, int clientPort,
                        int serverRtpSockfd, int serverRtcppSockfd)
{
    ...
    while(1)
    {
        ...
        if(!strcmp(method, "PLAY"))
        {
            ...
            rtp::rtpHeaderInit(rtpPacket, 0, 0, 0, RTP_VESION, RTP_PAYLOAD_TYPE_AAC, 1, 0, 0, 0x32411);
