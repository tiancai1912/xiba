...
int x264Encoder::init(EncodingFormat format) {
    ...
    if( x264_param_apply_profile(&mX264Param, "high" ) < 0 )
        return -1;
...
x264Encoder::Out * x264Encoder::encodeOneFrame(char *in, int len) {
    ...
    buffer->data = (char *)(mNalu->p_payload);
    return buffer;
}
...
//        mPicIn.img.plane[2] = (uint8_t *)bufV;
        ...
        buffer->data = (char *)(mNalu[0].p_payload);
        buffer->len = frameSize;

        qDebug() << "encode one frame : " << frameSize << endl;
        if (mCallback != nullptr) {
            mCallback->onData(buffer->data, buffer->len);
        }
