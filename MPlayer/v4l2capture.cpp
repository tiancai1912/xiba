...
QString v4l2Capture::enumDevices(const char *dev_name)
{
    int fd;
    ...
    if (cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) {
        ...
        printf("not video cap device");
        return QString("");
    }

    QString info;
    info.sprintf("Driver name: %s\n", cap.driver);
    ...
    for (int i = 0; ; i++) {
        ...
        QString pic_fmt;
        pic_fmt.sprintf("Pic name: %s\n", fmt.description);
        info += pic_fmt;

//        info.sprintf("%s, Pic name: %s\n", info, fmt.description);

        printf("dev pic format: %s", fmt.description);
    }

    close(fd);

//    QString info;
//    info.sprintf("Driver name: %s\n Pic name: %s\n", cap.driver, fmt.description);
//    info.append("Driver name:");
//    info.append(QString(cap.driver));
//    info.append("Pic format:");
//    info.append(fmt.description);

    return info;
}

const char *v4l2Capture::captureVideoFrame()
{
...
bool v4l2Capture::init(QString dev_name)
{
    openDevice(dev_name);
    initDevice();
...
//        for(;;) {
            ...
            m_callback->onFrameCapture((const char *)(buffers[buf.index].start), buf.bytesused, buf.index);
...
const char* v4l2Capture::getOneFrame()
{
    unsigned int count;
    count = 1;
    QString path = "";
...
QString v4l2Capture::storeImage(const char *buf_start, int size, int index)
{
    char path[20];
...
int v4l2Capture::readFrame(QString &path)
{
    struct v4l2_buffer buf;
    unsigned int i;
    ...
    case IO_METHOD_MMAP:
        ...
        path = storeImage((char *)(buffers[buf.index].start), buf.bytesused, buf.index);
...
void v4l2Capture::openDevice(QString dev_name)
{
    struct stat st;
    const char *name = dev_name.toLatin1().data();
    if (-1 == stat(name, &st)) {
...
void HandleFrame::onFrameCapture(const char *data, int length, int index)
{
    qDebug() << "hello onFrameCapture\n";
}
